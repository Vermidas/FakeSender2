...
async function sendenbutton() {
    try {
        let angriffeSendenIntervall = angriffeSenden();
        const urls = await sendAttacks(attacksDataTable);

        const endIndex = Math.min(currentAttackIndex + angriffeSendenIntervall, urls.length);
        ...
        function openTab(index) {
            if (index < endIndex) {
                ...
                let row = document.getElementById('attackRow-' + index);
                if (row) {
        ...
        openTab(currentAttackIndex); // Startet das Öffnen der Tabs
        currentAttackIndex = endIndex; // Aktualisiert den Index für den nächsten Durchlauf
    } catch (error) {
        console.error('Fehler beim Verarbeiten der Angriffe:', error);
    }
} if (window.location.href.includes('screen=memo')) {
document.getElementById('senden').addEventListener('click', sendenbutton);
}
}
...
function abschicken() {
    ...
    function checkAndReload() {
      ...
      var hasScreenPlace2 = url.includes("screen=place&village");
      var hasScreenPlace3 = url.includes("screen=place&try=confirm");
      ...
      $(document).ready(function() {
        if ($(".error_box").text().includes("Angriffstrupp")) {
            window.close();
        }
    });
