...
function abschicken() {
  ...
  const hasVillageItem = document.getElementsByClassName("village-item").length > 0;
  const hasScreenPlace = url.includes("screen=place&target") || url.includes("screen=place&village");
  ...
  if (tabName.includes("Fake Sender") || localStorage.getItem(village.id) !== null) {
    ...
    if ((hasScreenPlace2 && !hasVillageItem) || (hasVillageItem && hasScreenPlaceConfirm)) {
      setTimeout(() => {
        performAction("target_attack");
        localStorage.removeItem(village.id);
        window.close();
      }, Math.floor(Math.random() * (ACTION_DELAY_MAX - ACTION_DELAY_MIN)) + ACTION_DELAY_MIN);
    }

    const errorBox = document.querySelector(".error_box");
    if (errorBox && errorBox.textContent.includes("Angriffstrupp")) {
      localStorage.removeItem(village.id);
      window.close();
    }
  }
